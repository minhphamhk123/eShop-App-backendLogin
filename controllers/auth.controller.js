  ...
  Verification.find({ userId })
    .then((result) => {
      if (result.length > 0) {
          ...
          Verification.deleteOne({ userId })
            .then(() => {
              User.findByIdAndDelete(userId)
                ...
                .catch((error) => {
                  return next(errorHandler(404, "Can't delete User when clearing expires email"));
                });
            })
            .catch((error) => {
              return next(errorHandler(404, "Can't delete Verification when verify expires"));
            });
        } else {
          bcryptjs
            ...
            .then((result) => {
              if (result) {
                User.findByIdAndUpdate({ _id: userId }, { verified: true })
                  .then(() => {
                    Verification.deleteOne({ userId }).then(
                      res.status(201).json({ message: "User created and verify successfully" })
                    );
                    eventEmitter.emit("userVerifiedStatusChanged", { _id: userId, verified: true });
                  })
                  .catch((error) => {
                    return next(errorHandler(404, "Problem with update user"));
                  });
              } else {
        ...
        return next(
          errorHandler(404, "Account record doesn't exist or have been verified already. Please signup or login.")
        );
      }
    })
    .catch((error) => {
